...
export default boot(({ router, store }) => {
  // use injected store when available, otherwise fall back to default import
  const appStore = store ?? defaultStore
  ...
  router.beforeEach(async (to) => {
    const isAuthenticated = await appStore.dispatch('auth/check')
